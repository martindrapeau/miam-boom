(function() {
  ...
  Backbone.Hero = Backbone.Character.extend({
    ...
    onAttach: function() {
      Backbone.Character.prototype.onAttach.apply(this, arguments);
      if (this.input) {
    ...
    onDetach: function() {
      Backbone.Character.prototype.onDetach.apply(this, arguments);
      this.stopListening();
    },
    ...
    hit: function(sprite, dir, dir2) {
      ...
      if (type == "artifact") {
        ...
        if (this.isAttacking(this) && cur.dir == dir) {
          sprite.trigger("hit", this, cur.opo);
        } else {
          if (sprite.isAttacking(this)) {
            this.cancelUpdate = true;
    ...
    update: function(dt) {
      ...
      function updateTopBottom() {
        ...
        if (!ignorePhysics)
          hero.world.findCollisions(hero.collisionMap, hero.get("collideWith"), hero, true);
      }
      updateTopBottom();
      ...
      if (yVelocity >= 0) {
        ...
        if (yVelocity > 0 && heroBottomY >= bottomY) {
          ...
          if (this.input && !this.input.leftPressed() && !this.input.rightPressed())
            attrs.velocity = velocity = velocity/2;
        }
      ...
      if (velocity >= 0) {
        ...
        var rightX = this.world.width() + worldMargin;
        if (cur.mov != "dead")
